...
class IkFastSolver : public IkSolverBase
{
 ...
 public:
    ...
    bool SolveAll(const IkParameterization& param, int filteroptions, std::vector< std::vector<dReal> >& solutions) { return false; }
    bool SolveAll(const IkParameterization& param, const std::vector<dReal>& vFreeParameters, int filteroptions, std::vector< std::vector<dReal> >& solutions) { return false; }
    virtual bool Init(OpenRAVE::RobotBase::ManipulatorConstPtr pconstmanip){RobotBase::ManipulatorPtr pmanip =
boost::const_pointer_cast<RobotBase::Manipulator>(pconstmanip); Init(pmanip);}
#endif
...
//                RAVELOG_INFO(ss.str());
    ...
    SolutionResults _ValidateSolutionSingle(const Solution& iksol, boost::tuple<const vector<IKReal>&, const vector<dReal>&,int>& freeq0check, std::vector<IKReal>& sol, std::vector<dReal>& vravesol, std::vector<dReal>& vbest, dReal& bestdist, const IkParameterization& param, bool& bCheckEndEffector)
    {
        ...
        if( !!_filterfn ) {
           switch(_filterfn(vravesol, pmanip, param)._action) {
            case IKRA_Reject: return SR_Continue;
            case IKRA_Quit: return SR_Quit;
            case IKRA_Success:
                break;
            }
        }
